...
class AtomFeed extends Feed {
	...
	/**
	 * @type {Array<import('./item/atom')> | null}
	 */
	#itemCache = null;
	...
	get image() {
		const url = (
			this.#root.findElementWithName('logo')?.textContentAsUrl ||
			this.#root.findElementWithName('icon')?.textContentAsUrl ||
			null
		);
		if (url) {
			return {
	...
	/**
	 * @override
	 * @returns {Array<import('./item/atom')>}
	...
	get items() {
		...
		const items = this.#root.findElementsWithName('entry').map(entry => {
