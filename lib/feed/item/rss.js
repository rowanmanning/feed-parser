...
module.exports = class RssFeedItem extends FeedItem {
	...
	get media() {
		...
		return this.element.findElementsWithName('content')
			.filter(content => content.namespace === 'media')
			.map(RssFeedItem.#parseMediaItem)
			.filter(isNotNull);
	}
	...
	static #parseMediaItem(mediaItem) {
		...
		const rawLength = Number(mediaItem.getAttribute('length')) || null;
		const length = Number.isNaN(rawLength) || !rawLength ? null : rawLength;
